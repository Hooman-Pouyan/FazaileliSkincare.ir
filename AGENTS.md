...
- **Content lives in the database, and images have one address.** Editorial copy — FAQ answers, campaign bands, galleries, Landing beats — is read from the four `content_block` tables, never from a JSON import at runtime and never from a literal in a component. Image URLs come from `mediaUrl(objectKey)` in `src/lib/media/url.ts`; nothing else builds an image path, so moving to the CDN stays one environment variable. Both are `docs/26-content-and-catalogue-decisions.md`, `C-7` and `C-11`.

- **Seed data is real where it can be and marked where it cannot.** Truth is per field, not per row: real brands, products, forms, pack sizes and imagery; invented price, stock, SKU and claim, each carrying a marker a query can see — `DEMO-` on the SKU, `reviewState: draft` on the row. A comment protects the next reader of the file; a marker protects the next reader of the database, who is the one who ships. `C-1` and `C-3`.
