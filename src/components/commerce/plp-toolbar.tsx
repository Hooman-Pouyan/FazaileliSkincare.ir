...
export function PlpToolbar({
  resultCount, sort, onSortChange, inStockOnly, onInStockChange,
  sortOptions = DEFAULT_SORT_OPTIONS, children, className,
}: {
  resultCount: number;
  sort: string;
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-4 border-b border-[var(--hairline-soft)] pb-5", className)}>
      <p className="text-[14px] text-[var(--stone-text)] tabular-nums">
        <span className="font-medium text-[var(--ink)]">{resultCount}</span> محصول
      </p>
      ...
      <div className="ms-auto flex items-center gap-3">
        {children}
        <SortChips options={sortOptions} value={sort} onChange={onSortChange} className="hidden md:flex" />
      </div>
    </div>
  );
}
