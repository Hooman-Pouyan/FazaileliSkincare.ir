...
export function Pagination({
  page, pageCount, hrefFor, className,
}: { page: number; pageCount: number; hrefFor: (p: number) => string; className?: string }) {
  ...
  return (
    <nav aria-label="صفحه‌بندی" className={cn("flex items-center justify-center gap-1.5 py-12", className)}>
      {pages.map((p, i) => {
        const gap = i > 0 && p - pages[i - 1]! > 1;
        return (
          <span key={p} className="flex items-center gap-1.5">
            {gap && <span className="px-1 text-[var(--stone-text)]" aria-hidden>…</span>}
            <Link
              href={hrefFor(p)}
