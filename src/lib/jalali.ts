...
const FA_MONTHS = ["فروردین","اردیبهشت","خرداد","تیر","مرداد","شهریور","مهر","آبان","آذر","دی","بهمن","اسفند"] as const;
const FA_WEEKDAYS = ["شنبه","یک‌شنبه","دوشنبه","سه‌شنبه","چهارشنبه","پنج‌شنبه","جمعه"] as const;
...
export function toJalali(date: Date): JalaliParts {
  ...
  }).format(date).split("-").map(Number) as [number, number, number];
...
export function formatJalali(
  ...
  return new Intl.DateTimeFormat("fa-IR-u-ca-persian", { timeZone: TEHRAN, ...opts }).format(date);
}
...
export function jalaliMonthName(jm: number): string { return FA_MONTHS[jm - 1]!; }
export function jalaliWeekdayName(index: number): string { return FA_WEEKDAYS[index]!; }
export function daysInJalaliMonth(jy: number, jm: number): number { return jalaaliMonthLength(jy, jm); }
export function persianNumber(n: number): string { return toPersianDigits(String(n)); }
