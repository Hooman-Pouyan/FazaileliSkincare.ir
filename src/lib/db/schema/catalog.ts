...
export const productSkinState = pgTable("product_skin_state", {
  productId: uuid().notNull().references(() => product.id, { onDelete: "cascade" }),
  skinStateId: uuid().notNull().references(() => skinState.id, { onDelete: "cascade" }),
}, (table) => [
  primaryKey({ columns: [table.productId, table.skinStateId] }),
  index("product_skin_state_state_idx").on(table.skinStateId, table.productId),
]);

export const productProtocolPhase = pgTable("product_protocol_phase", {
  productId: uuid().notNull().references(() => product.id, { onDelete: "cascade" }),
  protocolPhaseId: uuid().notNull().references(() => protocolPhase.id, { onDelete: "cascade" }),
}, (table) => [
  primaryKey({ columns: [table.productId, table.protocolPhaseId] }),
  index("product_protocol_phase_phase_idx").on(table.protocolPhaseId, table.productId),
]);
...
export const productMedia = pgTable("product_media", {
  ...
  uniqueIndex("product_media_primary_unique").on(table.productId).where(sql`${table.role} = 'primary'`),
  ...
  check("product_media_checksum_check", sql`${table.checksumSha256} ~ '^[0-9a-f]{64}$'`),
  check("product_media_dimensions_check", sql`${table.width} > 0 and ${table.height} > 0 and ${table.byteSize} > 0`),
]);

export const productMediaTranslation = pgTable("product_media_translation", {
  productMediaId: uuid().notNull().references(() => productMedia.id, { onDelete: "cascade" }),
  localeCode: text().notNull().references(() => locale.code, { onDelete: "restrict" }),
  altText: text().notNull(),
}, (table) => [primaryKey({ columns: [table.productMediaId, table.localeCode] })]);

export const variant = pgTable("variant", {
  id: uuid().primaryKey().defaultRandom(),
  productId: uuid().notNull().references(() => product.id, { onDelete: "restrict" }),
  sku: text().notNull(),
  barcode: text(),
  ...
  uniqueIndex("variant_barcode_unique").on(table.barcode).where(sql`${table.barcode} is not null`),
...
export const variantTranslation = pgTable("variant_translation", {
