...
export const customerOrder = pgTable("customer_order", {
  ...
  subtotalRials: bigint({ mode: "bigint" }).notNull().default(sql`0`),
  shippingRials: bigint({ mode: "bigint" }).notNull().default(sql`0`),
  discountRials: bigint({ mode: "bigint" }).notNull().default(sql`0`),
  totalRials: bigint({ mode: "bigint" }).notNull().default(sql`0`),
  ...
  uniqueIndex("customer_order_checkout_idempotency_unique").on(table.checkoutIdempotencyKey),
  index("customer_order_person_time_idx").on(table.personId, table.createdAt, table.id),
  index("customer_order_status_time_idx").on(table.status, table.createdAt, table.id),
  check("customer_order_contact_check", sql`${table.personId} is not null or ${table.guestPhone} is not null`),
  check(
  ...
  check("customer_order_discount_check", sql`${table.discountRials} <= ${table.subtotalRials} + ${table.shippingRials}`),
  check("customer_order_version_check", sql`${table.version} >= 0`),
]);

export const orderLine = pgTable("order_line", {
  id: uuid().primaryKey().defaultRandom(),
  orderId: uuid().notNull().references(() => customerOrder.id, { onDelete: "restrict" }),
  ...
  check("order_line_total_check", sql`${table.lineTotalRials} = ${table.unitPriceRials} * ${table.quantity}`),
]);
