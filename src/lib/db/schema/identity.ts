...
export const locale = pgTable("locale", {
  ...
  uniqueIndex("locale_one_primary_unique").on(table.isPrimary).where(sql`${table.isPrimary}`),
]);
...
export const person = pgTable("person", {
  ...
  firstName: text(),
  lastName: text(),
  preferredLocaleCode: text().notNull().default("fa").references(() => locale.code, { onDelete: "restrict" }),
  createdAt: timestamp({ withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp({ withTimezone: true }).notNull().defaultNow(),
}, (table) => [
  uniqueIndex("person_email_unique").on(sql`lower(${table.email})`),
  uniqueIndex("person_phone_unique").on(table.phone).where(sql`${table.phone} is not null`),
  index("person_preferred_locale_idx").on(table.preferredLocaleCode),
  check("person_verified_phone_check", sql`not ${table.phoneVerified} or ${table.phone} is not null`),
  check("person_placeholder_email_check", sql`not ${table.emailIsPlaceholder} or not ${table.emailVerified}`),
]);
...
export const address = pgTable("address", {
  ...
  uniqueIndex("address_person_default_unique").on(table.personId).where(sql`${table.isDefault}`),
