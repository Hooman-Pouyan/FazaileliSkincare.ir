...
export const price = pgTable(
  ...
  (table) => [
    uniqueIndex("price_variant_group_unique").on(
      table.variantId,
      table.customerGroup,
    ),
...
export const priceAdjustmentBatch = pgTable(
  ...
  (table) => [
    uniqueIndex("price_adjustment_batch_request_hash_unique").on(
      table.requestHash,
    ),
...
export const priceHistory = pgTable(
  "price_history",
  {
    ...
    changedBy: uuid()
      .notNull()
      .references(() => person.id, { onDelete: "restrict" }),
    batchId: uuid().references(() => priceAdjustmentBatch.id, {
      onDelete: "restrict",
    }),
    changedAt: timestamp({ withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    index("price_history_variant_changed_idx").on(
      table.variantId,
      table.changedAt,
    ),
...
export const inventoryMovement = pgTable(
  ...
  (table) => [
    uniqueIndex("inventory_movement_idempotency_unique").on(
      table.idempotencyKey,
    ),
    index("inventory_movement_variant_time_idx").on(
      table.variantId,
      table.occurredAt,
    ),
    index("inventory_movement_aggregate_idx").on(
      table.relatedAggregateType,
      table.relatedAggregateId,
    ),
    index("inventory_movement_actor_idx").on(table.actorId),
    check("inventory_movement_delta_check", sql`${table.quantityDelta} <> 0`),
    check(
      "inventory_movement_result_check",
      sql`${table.resultingOnHand} >= 0`,
    ),
    check(
