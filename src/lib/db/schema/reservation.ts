...
export const inventoryReservation = pgTable("inventory_reservation", {
  ...
  sourceCartItemId: uuid().notNull().references(() => cartItem.id, { onDelete: "restrict" }),
  orderLineId: uuid().references(() => orderLine.id, { onDelete: "restrict" }),
  ...
  uniqueIndex("inventory_reservation_idempotency_unique").on(table.idempotencyKey),
