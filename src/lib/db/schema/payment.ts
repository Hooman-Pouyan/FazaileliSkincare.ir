...
export const payment = pgTable("payment", {
  ...
  uniqueIndex("payment_provider_authority_unique")
    ...
    .where(sql`${table.provider} is not null and ${table.providerAuthority} is not null`),
  ...
  index("payment_status_time_idx").on(table.status, table.createdAt, table.id),
...
export const bankTransferClaim = pgTable("bank_transfer_claim", {
  id: uuid().primaryKey().defaultRandom(),
  paymentId: uuid().notNull().references(() => payment.id, { onDelete: "restrict" }),
  ...
  uniqueIndex("bank_transfer_claim_submission_unique").on(table.submissionIdempotencyKey),
  index("bank_transfer_claim_payment_time_idx").on(table.paymentId, table.createdAt),
  index("bank_transfer_claim_status_time_idx").on(table.status, table.createdAt, table.id),
  index("bank_transfer_claim_reviewer_idx").on(table.reviewedBy),
  check("bank_transfer_claim_amount_check", sql`${table.expectedAmountRials} >= 0`),
  check("bank_transfer_claim_last4_check", sql`${table.last4OfCard} is null or ${table.last4OfCard} ~ '^[0-9]{4}$'`),
  check(
...
export const paymentSettlement = pgTable("payment_settlement", {
  ...
  uniqueIndex("payment_settlement_idempotency_unique").on(table.idempotencyKey),
