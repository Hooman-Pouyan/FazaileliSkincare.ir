...
## Next 16 differs from what a model remembers

`next dev` writes an agent-rules block into `AGENTS.md` pointing at
`node_modules/next/dist/docs/`. Read it before writing route code. The changes
that matter here:

- **`params` and `searchParams` are Promises.** Synchronous access was removed in
  16; every route and layout must `await` them. The existing routes already do.
- **`PageProps<'/route'>` and `LayoutProps<'/route'>`** are generated by
  `next typegen` into `.next/types` and are preferable to hand-written prop types.
- **Parallel route slots require an explicit `default.js`** or the build fails.
- **`scroll-behavior: smooth` is no longer overridden** during navigation unless
