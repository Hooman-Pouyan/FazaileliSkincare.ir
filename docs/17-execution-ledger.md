...
| 7   | PLP and search                                                                                                                                                                                                                              | Stage 3, `PLP0–5`                                                     | Concern, brand, category and `?q=` routes use canonical URL state, server results, live counts, stable pagination, verified empty and error states                                                                                                                                                                                                                                                                                                                                                              | **done** — concern, brand, category and `?q=` routes render from PostgreSQL with canonical URL state and live counts; facet counts closed in this packet. Verified in a browser at 390/768/1440, review item 7.11. Two SEO findings from that pass, 7.12 and 7.13, were fixed in `9eb7835` |
...
| 8   | PDP                                                                                                                                                                                                                                         | Stage 4, `PDP1–6`                                                     | Offer states are truthful; `on_request`, professional-only, unavailable and variant-required cannot enter the cart                                                                                                                                                                                                                                                                                                                                                                                              |                                                                                                                                                                                                                                                                                            | **done** — `/shop/p/[slug]` live, all 48 listed products resolve, offer states exhaustive, `Product` JSON-LD leaks no restricted price. Packet 8 review log                       |
| 9   | Cart presentation — drawer and `/fa/cart`                                                                                                                                                                                                   | Stage 4, `CART1`                                                      | The shared cart model renders in both surfaces; no checkout, payment or settlement code exists                                                                                                                                                                                                                                                                                                                                                                                                                  | **done** — drawer and `/cart` on one model; `/cart` correct with JavaScript off. Packets 9/10 review log                                                                                                                                                                                   |
| 10  | Transactional cart, reservations, `resolveCartOwner`                                                                                                                                                                                        | DB5, C5                                                               | Concurrent requests cannot oversell; removal works; retries and guest-to-account merge are idempotent; the expiry predicate is observable                                                                                                                                                                                                                                                                                                                                                                       | **done** — migration `0007` (`C5`), `FOR UPDATE` serialisation, 13 integration tests; six contending carts cannot oversell                                                                                                                                                                 |
...
| 14  | Checkout and order placement         | `COM2`, `COM3`             | One server quote resolved by specificity (11 unit tests); `placeOrder` atomic, idempotent and server-priced (9 integration tests, including genuine concurrency). A complete purchase driven in a browser: `FZ-20260827-8RVVO`                             |
| 15  | Bank transfer and staff verification | `COM4`                     | A claim moves nothing; only a staff confirmation settles. Ten integration tests plus a live run through the real screens: stock 7 → 6, one `sale` movement, reservation consumed, `payment.settled` audited, `order.paid` queued. Bank details fail closed |
...
| Packet 15 — bank transfer and staff verification — 2026-08-27  | 15.10 | Bank details fail closed                                                               | content           |
| Packet 15 — bank transfer and staff verification — 2026-08-27  | 15.14 | The dev server currently holds bank details in its environment                         | process           |
| Packet 14 — checkout and order placement — 2026-08-27          | 14.10 | Development shipping rates are seeded, and fenced three ways                           | content           |
...
| Packet 11 — density — 2026-08-26                               | 11.3  | The height cap was the wrong axis, and the maintainer caught it                        | design            |
| Packet 11 — density — 2026-08-26                               | 11.4  | Type is deliberately unchanged, and the design system disagrees                        | design            |
| Packets 9 and 10 — the cart and its reservations — 2026-08-26  | 9.7   | The cart entry is now visible in the rail and the bottom bar                           | UI                |
...
| Packet 15 — bank transfer and staff verification — 2026-08-27  | 15.12 | Late funds against gone stock are refused, not forced                                                                                 | product         |
| Packet 14 — checkout and order placement — 2026-08-27          | 14.12 | Guest checkout is not built                                                                                                           | product         |
...
**Totals:** 54 waiting on the maintainer, 54 open.
